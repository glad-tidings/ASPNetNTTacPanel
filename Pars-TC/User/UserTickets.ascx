<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="UserTickets.ascx.vb" Inherits="Pars_TC.UserTickets" %>
...
<%If QSTRID <> 0 Then%>
<h2>کنترل پنل کاربران</h2><div class='featured'><h3>مشخصات درخواست</h3>
<%Try
        ...
        Do While d.Read
            If d("TSender".ToString) <> Session.Item("VUser") Then Response.Redirect("/User/?Page=UserTickets")
...
<%Else%>
<h2>کنترل پنل کاربران</h2><div class='featured'><h3>مشاهده درخواستهای پشتیبانی</h3>
<asp:Repeater ID="RepeaterNews" runat="server">
                ...
                <ItemTemplate>
                    <div class="DivTicket">
                        <%Select Case BGC()
                                Case True%>
                        <a style="height: 22px; display: table-row;" href="?Page=UserTickets&ID=<%# DataBinder.Eval(Container.DataItem, "ID")%>">
                        <%Case false%>
                        <a style="height: 22px; background-color: #dcdcdc; display: table-row;" href="?Page=UserTickets&ID=<%# DataBinder.Eval(Container.DataItem, "ID")%>">
                        <%End select%>
